...
void ExecutorTestes::testarAVL(const std::vector<int>& dados, const std::string& dataset, size_t n) {
    ...
    resInsert.tempoTotalSegundos = tempoTotal;
    resInsert.tempoMedioNanossegundos = (tempoTotal * 1e9) / n;
    resInsert.rotacoes = arvore.getNumRotacoes();
    resInsert.comparacoes = arvore.getNumComparacoes();
    resInsert.memoria = arvore.getConsumoMemoria();
    resInsert.altura = arvore.calcularAlturaAVL();
    ...
    resSearch.tempoTotalSegundos = tempoTotal;
    resSearch.tempoMedioNanossegundos = (tempoTotal * 1e9) / n;
    resSearch.rotacoes = arvore.getNumRotacoes();
    resSearch.comparacoes = arvore.getNumComparacoes();

    GerenciadorArquivos::salvarResultadoCSV(CAMINHO_CSV, resSearch);

    // --- REMOÇÃO (50% dos elementos) ---
    size_t limiteRemocao = n / 2;
    inicio = std::chrono::high_resolution_clock::now();
    for (size_t i = 0; i < limiteRemocao; ++i) {
        arvore.deletarElemento(dados[i]);
    }
    fim = std::chrono::high_resolution_clock::now();
    tempoTotal = std::chrono::duration<double>(fim - inicio).count();
    ...
    resDelete.tempoTotalSegundos = tempoTotal;
    resDelete.tempoMedioNanossegundos = (limiteRemocao > 0) ? (tempoTotal * 1e9) / limiteRemocao : 0;
    resDelete.rotacoes = arvore.getNumRotacoes();
    resDelete.comparacoes = arvore.getNumComparacoes();
    resDelete.altura = arvore.calcularAlturaAVL();
...
void ExecutorTestes::testarBST(const std::vector<int>& dados, const std::string& dataset, size_t n) {
    ...
    if (dataset == "int_ordenado" && n > 10000) {
    ...
    resInsert.comparacoes = arvore.getNumComparacoes();
    resInsert.memoria = arvore.getConsumoMemoria();
    resInsert.altura = arvore.calcularAlturaBST();
    resInsert.observacao = "Sucesso";
    ...
    resDelete.tempoMedioNanossegundos = (limiteRemocao > 0) ? (tempoTotal * 1e9) / limiteRemocao : 0;
    resDelete.comparacoes = arvore.getNumComparacoes();
    resDelete.altura = arvore.calcularAlturaBST();
...
void ExecutorTestes::testarSplay(const std::vector<int>& dados, const std::string& dataset, size_t n) {
    ...
    resInsert.comparacoes = arvore.getNumComparacoes();
    resInsert.memoria = arvore.getConsumoMemoria();
    resInsert.altura = arvore.alturaSplay();
    resInsert.observacao = "Sucesso";
    ...
    resDelete.rotacoes = arvore.getNumRotacoes();
    resDelete.comparacoes = arvore.getNumComparacoes();
    resDelete.altura = arvore.alturaSplay();
...
void ExecutorTestes::testarTreap(const std::vector<int>& dados, const std::string& dataset, size_t n) {
    ...
    resInsert.comparacoes = arvore.getNumComparacoes();
    resInsert.memoria = arvore.getConsumoMemoria();
    resInsert.altura = arvore.alturaTreap();
    resInsert.observacao = "Sucesso";
    ...
    resDelete.rotacoes = arvore.getNumRotacoes();
    resDelete.comparacoes = arvore.getNumComparacoes();
    resDelete.altura = arvore.alturaTreap();
...
void ExecutorTestes::testarTrie(const std::vector<std::string>& dados, const std::string& dataset, size_t n) {
    ...
    // --- INSERÇÃO ---
    auto inicio = std::chrono::high_resolution_clock::now();
    for (const auto& str : dados) {
        arvore.inserirElemento(str);
    }
    auto fim = std::chrono::high_resolution_clock::now();
    double tempoTotal = std::chrono::duration<double>(fim - inicio).count();
    ...
    resInsert.comparacoes = arvore.getNumComparacoes();
    resInsert.memoria = arvore.getConsumoMemoria();
    resInsert.altura = arvore.alturaTrie();
    resInsert.observacao = "Sucesso";
...
void ExecutorTestes::testarPatricia(const std::vector<std::string>& dados, const std::string& dataset, size_t n) {
    ...
    resInsert.comparacoes = arvore.getNumComparacoes();
    resInsert.memoria = arvore.getConsumoMemoria();
    resInsert.altura = arvore.alturaPatricia();
    resInsert.observacao = "Sucesso";
...
void ExecutorTestes::testarKDTree(const std::vector<std::vector<double>>& pontos, const std::string& dataset, size_t n, int dimensao) {
    ...
    resInsert.operacao = "Insercao";
    resInsert.tamanhoN = n;
    resInsert.tempoTotalSegundos = std::chrono::duration<double>(endInsert - startInsert).count();
    resInsert.tempoMedioNanossegundos = (resInsert.tempoTotalSegundos * 1e9) / n;
    resInsert.rotacoes = 0;
    resInsert.comparacoes = arvore.getNumComparacoes();
    resInsert.memoria = arvore.getConsumoMemoria();
    resInsert.altura = arvore.alturaKdTree();
    ...
    for (const auto& p : pontos) {
        arvore.buscarElemento(p);
    }
    auto endSearch = std::chrono::high_resolution_clock::now();

    Resultado resSearch = resInsert;
    resSearch.operacao = "Busca";
    resSearch.tempoTotalSegundos = std::chrono::duration<double>(endSearch - startSearch).count();
    resSearch.tempoMedioNanossegundos = (resSearch.tempoTotalSegundos * 1e9) / n;
    resSearch.comparacoes = arvore.getNumComparacoes();
    ...
    for (size_t i = 0; i < n / 2 && i < pontos.size(); ++i) {
        arvore.deletarElemento(pontos[i]);
    }
    auto endDelete = std::chrono::high_resolution_clock::now();

    Resultado resDelete = resInsert;
    resDelete.operacao = "Remocao";
    resDelete.tamanhoN = n / 2;
    resDelete.tempoTotalSegundos = std::chrono::duration<double>(endDelete - startDelete).count();
    resDelete.tempoMedioNanossegundos = (resDelete.tempoTotalSegundos * 1e9) / (n / 2);
    resDelete.comparacoes = arvore.getNumComparacoes();
    resDelete.altura = arvore.alturaKdTree();
...
void ExecutorTestes::testarTrie(const std::vector<std::string>& dados, const std::string& dataset, size_t n) {
    ...
    for (int r = 0; r < NUM_RODADAS; ++r) {
        ...
        if (r == NUM_RODADAS - 1) comparacoesDelete = arvore.getNumComparacoes();
    }
